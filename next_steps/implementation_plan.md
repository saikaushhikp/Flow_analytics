...
2. Port Brussels crosswalk cleanup to Oulu.
...
3. Fix Oulu lane default behavior.
...
2. Or explicitly mark SPF as experimental-disabled.
   - Update README/config docs.
   - Keep code but do not present it as runnable.

Recommendation: mark SPF as experimental-disabled for now unless SPF is a near-term deliverable. M-DRAC and IRSM need attention first.

## Milestone 4: Repair VLM Validation
...
3. Fix Oulu preprocessing import.
...
- VLM response parser handles expected structured responses.
...
1. Re-run Brussels lanes and crosswalks for a small verified window.
2. Re-run Oulu lanes and crosswalks after fixing Oulu scripts.
3. Regenerate plots only for selected detections.
4. Run VLM validation on top-risk events if API key/model is available.
5. Produce a cross-region summary:
   - detections per day
   - detections per zone
   - severity/MDRAC distribution
   - false-positive notes from manual/VLM review
...
- One-day Oulu lane and crosswalk scripts run.
